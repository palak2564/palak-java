    ...
    public static void main(String[] args) {
        try {
            ...
            try (Statement stmt = con.createStatement()) {
                ...
                System.out.println("Database 'portfoliomanagementjava' created (or already exists).");
            }
...
private static void createTables(Connection con) {
    ...
    private static void userMenu(Connection con, Scanner scanner, String username) throws SQLException {
        while (true) {
            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    viewStocks(con, username, scanner);
                    viewBonds(con, username, scanner);
                    break;
                case 3:
                    // SUMIT KA CODE
    ...
    private static void createPortfolio(Connection con, Scanner scanner, String username) {
        try {
            ...
            String insertPortfolioSQL = "INSERT INTO portfolio (portfolio_name, username) VALUES (?, ?)";
...
private static void viewStocks(Connection connection, String username, Scanner scanner) {
    try {
        String query = "SELECT stock_symbol, buy_date, quantity FROM stocks WHERE username = ?";
...
private static void viewBonds(Connection connection, String username, Scanner scanner) {
    try {
        String query = "SELECT bond_name, purchase_date, face_value, bond_price, quantity FROM bonds WHERE username = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            pstmt.setString(1, username);
            ResultSet resultSet = pstmt.executeQuery();

            System.out.println("Bonds in your portfolio:");
            while (resultSet.next()) {
                String bondName = resultSet.getString("bond_name");
                String purchaseDate = resultSet.getString("purchase_date");
                double faceValue = resultSet.getDouble("face_value");
                double bondPrice = resultSet.getDouble("bond_price");
                int quantity = resultSet.getInt("quantity");
    ...
    private static void assetManagementMenu(Connection con, Scanner scanner, String usernameInput) throws SQLException {
        while (true) {
    ...
    private static void viewPortfolio(Connection connection, String username, Scanner scanner) throws SQLException {
        System.out.print("Enter portfolio name: ");
        String portfolioName = scanner.next();

        String query = "SELECT * FROM portfolio WHERE portfolio_name = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            pstmt.setString(1, portfolioName);
            ResultSet resultSet = pstmt.executeQuery();
    ...
    private static boolean isPortfolioNameTaken(Connection con, String portfolioName) {
        try {
            String checkPortfolioSQL = "SELECT COUNT(*) FROM portfolio WHERE LOWER(portfolio_name) = LOWER(?)";
